...
unique_tool_titles="$(awk -F'"' '/^                \("/ { print $2 }' "$SOURCE/Models/SafetyTool.swift" | LC_ALL=C sort -u | wc -l | tr -d ' ')"
...
grep -Fq 'home-search-field' "$SOURCE/Views/Tab1_Illustrations/IllustrationsHomeView.swift" \
  && pass "home search field is wired" \
  || fail "home search field missing"

grep -Fq 'store.search(query)' "$SOURCE/Views/Tab1_Illustrations/IllustrationsHomeView.swift" \
  && pass "home search uses ArticleStore results" \
  || fail "home search does not use ArticleStore"

grep -Fq 'case 2: GalleryView()' "$SOURCE/App/ContentView.swift" \
  && pass "gallery tab inserted after full text" \
  || fail "gallery tab order missing"

grep -Fq 'SeriesGalleryView(seriesId:' "$SOURCE/Views/Tab1_Illustrations/IllustrationsHomeView.swift" \
  && pass "five-category routes use gallery" \
  || fail "category gallery route missing"

grep -Fq 'PinterestArticleGrid' "$SOURCE/Views/Tab3_Gallery/GalleryView.swift" \
  && pass "Pinterest two-column gallery exists" \
  || fail "Pinterest gallery missing"

grep -Fq 'SafetyToolDetailView(tool:' "$SOURCE/Views/Tab4_Calculators/SafetyToolsView.swift" \
  && pass "all tool cards route to a functional detail" \
  || fail "tool detail route missing"

grep -Fq 'tool-risk-result' "$SOURCE/Views/Tab4_Calculators/SafetyToolsView.swift" \
...
if grep -Fq 'Configuration.storekit in Resources' "$ROOT/OSHFacilitiesIllustrated.xcodeproj/project.pbxproj"; then
